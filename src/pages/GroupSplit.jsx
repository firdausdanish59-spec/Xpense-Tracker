    ...
    <div style={{ paddingBottom: '5rem' }}>
      ...
      {activeTab === 'group' && (
        <div style={{ animation: 'fadeIn 0.3s' }}>
          ...
          {selectedGroup ? (
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.2fr 1.8fr', gap: '1.5rem' }}>
              <GlassCard>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
                  <div>
                    <h2 style={{ margin: '0 0 0.5rem', fontSize: '1.5rem' }}>{selectedGroup.name}</h2>
                    <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                      {selectedGroup.members.map((m, i) => (
                        <div key={i} title={m} style={{ width: '24px', height: '24px', borderRadius: '50%', background: MEMBER_COLORS[i % MEMBER_COLORS.length], display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '10px', fontWeight: 700, border: '2px solid var(--bg-card)' }}>
                          {m.trim().charAt(0)}
                        </div>
                      ))}
                    </div>
                    ...
                    {simplifiedDebts.map((debt, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.03)', padding: '0.85rem', borderRadius: 'var(--radius-btn)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <span style={{ fontWeight: 600, background: 'var(--gradient-2)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontSize: '0.85rem' }}>{debt.from}</span>
                          <ArrowRight size={14} color="var(--text-muted)" />
                          <span style={{ fontWeight: 600, background: 'var(--gradient-4)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontSize: '0.85rem' }}>{debt.to}</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--color-expense)', fontSize: '0.9rem' }}>₹{debt.amount.toFixed(0)}</span>
                          <Button onClick={() => handleMarkSettled(debt)} style={{ padding: '0.25rem 0.5rem', fontSize: '0.7rem', borderRadius: 'var(--radius-pill)' }}>
                            Mark Paid
                          </Button>
                        </div>
                      </div>
                ...
                {selectedGroup?.settlements && selectedGroup.settlements.length > 0 && (
                  <div style={{ marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid var(--border-color)' }}>
                    <h4 style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Recent Settlements</h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      {[...(selectedGroup.settlements || [])].sort((a,b) => new Date(b.date) - new Date(a.date)).slice(0, 5).map(s => (
                        <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.75rem', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.02)', padding: '0.5rem 0.75rem', borderRadius: 'var(--radius-btn)' }}>
                          <span><strong>{s.from}</strong> → <strong>{s.to}</strong></span>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            <span>₹{s.amount.toLocaleString()}</span>
                            <button disabled={isLoading} onClick={() => handleUndoSettlement(s.id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}><Trash2 size={12} /></button>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <GlassCard style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                  <h3 style={{ margin: 0, fontSize: '1rem' }}>Expenses</h3>
                  <Button onClick={() => { setNewExp({...newExp, includedMembers: selectedGroup.members.map(m => m.trim())}); setExpenseModalOpen(true); }} style={{ padding: '0.5rem 1rem', borderRadius: 'var(--radius-btn)', fontSize: '0.8rem' }}><Plus size={14} /> Add</Button>
                </div>
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.75rem', overflowY: 'auto', maxHeight: isMobile ? '400px' : '500px' }}>
                    ...
                    [...selectedGroup.expenses].reverse().map(ex => (
                      <div key={ex.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '0.75rem', borderBottom: '1px solid var(--border-color)' }}>
                        <div style={{ minWidth: 0 }}>
                          <h4 style={{ margin: '0 0 0.25rem', fontSize: '0.9rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ex.description}</h4>
                          <p style={{ margin: 0, fontSize: '0.7rem', color: 'var(--text-muted)' }}>{ex.paidBy} • {format(new Date(ex.date), 'MMM dd')}</p>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                          <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '0.95rem' }}>₹{ex.amount.toLocaleString()}</span>
                          <button disabled={isLoading} onClick={() => handleDeleteExpense(selectedGroup.id, ex.id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}><Trash2 size={14} /></button>
                        </div>
                      </div>
                    ))
                  )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                    {selectedGroup.members.map(m => {
                      const mem = m.trim();
                      const isIncluded = (newExp.includedMembers || selectedGroup.members.map(x => x.trim())).includes(mem);
                      return (
                        <button type="button" key={mem} onClick={() => {
                            ...
                            if (current.includes(mem)) {
                              ...
                              if (updated.length === 0) updated = [mem];
                            } else {
                              updated = [...current, mem];
                            }
